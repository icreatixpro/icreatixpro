...
const authors: Record<string, BlogAuthor> = {
  ...
  "Marcus Rodriguez": {
    ...
    bio: "AI researcher turned SEO architect with a PhD in Machine Learning.",
...
// Get single blog by slug - CRITICAL EXPORT
export function getBlogBySlug(slug: string): BlogPost | undefined {
  try {
    ...
    if (!matchedFile) {
      ...
      console.log(`Available slugs: ${files.map(f => normalizeSlug(f)).join(", ")}`);
      return undefined;
    }
...
export function searchPosts(query: string): BlogPost[] {
  ...
  return allPosts.filter((post) => {
    return (
      ...
      post.keywords.some(kw => kw.toLowerCase().includes(searchTerm))
    );
  });
}
